...
class FFTLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        label_fft = torch.fft.rfft2(target)
        loss = F.l1_loss(pred_fft, label_fft, reduction='none').mean()
        return loss * self.loss_weight, None

@LOSS_REGISTRY.register()
class FFTWeightLoss(nn.Module):

    def __init__(self, loss_weight=1.0, reduction='mean', eps=1e-12):
        super(FFTWeightLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. ' f'Supported ones are: {_reduction_modes}')
...
class PSNRLoss(nn.Module):
    ...
    def forward(self, pred, target, mask=None, weight=None):
        ...
        if mask is not None and weight is not None:
            loss += self.loss_weight * self.scale * torch.log(((pred*mask - target*mask) ** 2).mean(dim=(1, 2, 3)) + 1e-8).mean() * weight
            loss += self.loss_weight * self.scale * torch.log(((pred*(1 - mask) - target*(1 - mask)) ** 2).mean(dim=(1, 2, 3)) + 1e-8).mean() * (1-weight)
        else:
...
class MSEWeightLoss(nn.Module):
    ...
    def forward(self, pred, target, mask=None, weight=None):
        ...
        assert len(pred.size()) == 4
        if mask is not None and weight is not None:
            loss += self.loss_weight * ((pred*mask - target*mask) ** 2).mean() * weight
            loss += self.loss_weight * ((pred*(1 - mask) - target*(1 - mask)) ** 2).mean() * (1-weight)
        else:
            loss += self.loss_weight * ((pred - target) ** 2).mean()
        return loss
...
def _ssim(img1, img2, window, window_size, channel, size_average=False, weight=None):
...
@LOSS_REGISTRY.register()
class SSIMWeightLoss(torch.nn.Module):
    def __init__(self, loss_weight=1.0, window_size=11, size_average=False, reduction='mean'):
        super(SSIMWeightLoss, self).__init__()
        self.window_size = window_size
        self.size_average = size_average
        self.channel = 1
        self.loss_weight = loss_weight
    ...
    def forward(self, img1, img2, mask=None):
        ...
        return (1 - _ssim(img1, img2, window, self.window_size, channel, self.size_average, mask)) * self.loss_weight
...
class BCELoss(torch.nn.Module):
    ...
    def forward(self, img1, img2, mask=None):
        return self.bce(img1, img2) * self.loss_weight
